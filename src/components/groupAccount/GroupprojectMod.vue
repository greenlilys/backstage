		...
		<el-row>
			...
			<el-col class="main-all">
					...
					<el-form-item label="所在地区：">
						<template>
							<v-distpicker :province="form.provincename" :city="form.cityname" :area="form.countyname" @selected="onSelected"></v-distpicker>
						</template>
					</el-form-item>
				...
				this.$refs[formName].validate((valid) => {
					if (valid) {
						if(that.form.provincename==''||that.form.cityname==''||that.form.countyname==''){
							...
							that.$post('/groupPro/update',that.form).then(data=>{
								console.log(data)
								this.$ye('修改成功');
								this.$router.push('/Main/Groupprojectdetail?id='+that.id);
							})
						}
					} else {
						return false;
