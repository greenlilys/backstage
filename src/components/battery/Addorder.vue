<template>
	<div >
		...
		<el-row class="bw p-10 boxborder" type="flex" align="middle">
			...
			<div class="paddinglist boxborder pr bw mt-10 flex-con">					
				<template>
...
<script>
	...
	export default {
      data(){
      	...
      	methods:{
			handleRedio(v) {
				this.AddBattery({radio:v});
				this.currentPage = 1;
			},			
			handleCurrentChange(v) {
				this.AddBattery({pageNo:v});
			},	 
			Shment(index,row){
				this.currentId=row.id;
				this.dialogFormVisible = true;
			},   	
	    	AddBattery({pageNo=1,find=this.find,radio=this.radio}={}){			
